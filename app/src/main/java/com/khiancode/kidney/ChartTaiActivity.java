...
public class ChartTaiActivity extends AppCompatActivity {
    private Realm realm;
...
//        results = realm.where(BmiModel.class).sort("dt", Sort.ASCENDING).findAll();
        results = realm.where(TaiModel.class).sort("dt", Sort.ASCENDING).findAll();

        for (TaiModel model:results) {
